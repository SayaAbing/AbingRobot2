...
from AbingRobot.__main__ import STATS, TOKEN, USER_INFO
from AbingRobot.modules.sql import SESSION
import AbingRobot.modules.sql.userinfo_sql as sql
from AbingRobot.modules.disable import DisableAbleCommandHandler
from AbingRobot.modules.sql.global_bans_sql import is_user_gbanned
from AbingRobot.modules.sql.afk_sql import is_afk, set_afk
from AbingRobot.modules.sql.users_sql import get_user_num_chats
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>╔═━「 Current Abing Statistics 」</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)
    result += "\n<b>╘═━「 Powered By SayaAbing 」</b>"
    update.effective_message.reply_text(
        result,
        parse_mode=ParseMode.HTML, 
        disable_web_page_preview=True
